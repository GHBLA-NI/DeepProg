...
class LoadData():
    ...
    def __init__(
        ...
        self.cross_validation_instance =  KFold(n_splits=5, shuffle=True, random_state=1)
...
#         print('{0} loaded of dim:{1}'.format(f_name, matrix.shape))
    ...
    def create_a_cv_split(self):
        """ """ 
        #print(f"Initial sample IDs length: {len(self.sample_ids)}")
        ...
        #print(f"Train length after KFold split: {len(train)}")
        #print(f"Test length after KFold split: {len(test)}")
        ...
        if self.normalization['PERC_SAMPLE_TO_KEEP']:
            ...
            train = train[to_keep]
            print(f"Train length after applying PERC_SAMPLE_TO_KEEP: {len(train)}")
            print(f"Test length after applying PERC_SAMPLE_TO_KEEP: {len(test)}")
            
        for key in self.matrix_array:
            self.matrix_cv_array[key] = self.matrix_array[key][test]
            self.matrix_array[key] = self.matrix_array[key][train]
        #print(f"Matrix '{key}' train size: {self.matrix_array[key].shape[0]}")
        #print(f"Matrix '{key}' test size: {self.matrix_cv_array[key].shape[0]}")
        ...
        #print(f"test id length{len(self.sample_ids_cv)}")
        #print(f"training id length{len(self.sample_ids)}")
    ...
    def load_survival(self):
        ...
        for ids, sample in enumerate(self.sample_ids):
            if sample not in survival:
                sample_removed += 1
                if self.verbose: 
                    print(f"Sample '{sample}' (ID: {ids}) is being removed as it has no survival data.")
                continue
    ...
    def normalize_training_array(self):
        """ Normalize the training array while constructing Tau correlation matrix for drug data. """
        for key in self.matrix_array:
            #print(f"Processing key: {key}")

            matrix = self.matrix_array[key].copy()

            if key == 'drug':  # Adjust the key name if necessary
                print(f"Using Tau correlation for {key}")
                # Validate input matrix
                assert len(matrix.shape) == 2, f"Input matrix for {key} must be 2D, got shape {matrix.shape}"
                assert matrix.shape[0] > 0 and matrix.shape[1] > 0, "Matrix for drug key is empty or malformed"
                ...
                for i in range(num_samples):
                    for j in range(num_samples):
                        if i == j:
                            ...
                            tau_matrix[i, j] = 0.0 if np.isnan(tau) else tau

                # Store the Tau matrix in matrix_train_array
                self.matrix_train_array[key] = tau_matrix
            else:
        ...
        #if 'drug' in self.matrix_train_array:
            #print("Matrix train array for key 'drug':")
            #print(self.matrix_train_array['drug'])
        #else:
            #print("Key 'drug' not found in matrix_train_array.")
    ...
    def _normalize(self, matrix, key):
        """ Normalizes the matrix except for drug data. """
        if key == 'drug':  # Skip normalization for drug
            print(f"Skipping normalization for function _normalize for {key}")
            return matrix
