...
def coxph_from_python(
    ...
    #print("[DEBUG] Encoder predict values before CoxPH:", values)
    ...
    if do_KM_plot:
        ...
        for i, label in enumerate(set(values)):
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                kaplan.fit(
                    #values[values==label],
        ...
        ax.set_xlabel('Nb. days', fontsize=20)
        ...
        base_figname = "{0}/{1}".format(
            png_path, fig_name.replace('.pdf', '').replace('.png', ''))
        pdf_figname = base_figname + '.pdf'
        png_figname = base_figname + '.png'

        fig.savefig(pdf_figname)
        fig.savefig(png_figname)
        print(f'Figure saved in: {pdf_figname} and {png_figname}')
