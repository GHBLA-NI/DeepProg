...
class SimDeepBoosting():
    ...
    def _fit(self, debug=False, verbose=False, pretrained_labels_files=[]):
        ...
        try:
            ...
            if pretrained_labels_files:
                ...
                print("[DEBUG] Calling SimDeep.fit.")
                results = [
    ...
    def compute_feature_scores_per_cluster(self, pval_thres=1):
        """
        """
        ...
        def generator(labels, feature_list, matrix):
            """
            Generates input for parallel feature scoring.
            """
            for i in range(len(feature_list)):
                yield feature_list[i], matrix[i], labels, pval_thres

        # Retrieve feature dictionary and identify drug features
        feature_dict = self._from_model_dataset(self.models[0], 'feature_array')
        drug_features = feature_dict.get('drug', [])
        ...
        for key in self.matrix_with_cv_array:
            ...
            if len(drug_features) > matrix.T.shape[0]:
                print(f"Reducing drug_features from {len(drug_features)} to {matrix.T.shape[0]}")
                drug_features = drug_features[:matrix.T.shape[0]]

            drug_input_list = generator(labels, drug_features, matrix.T)

            # Score drug features
            drug_features_scored = mapf(
                _process_parallel_feature_importance_per_cluster, drug_input_list
            )
            drug_features_scored = [
                feat for feat_list in drug_features_scored for feat in feat_list
            ]

            # Add drug features to feature scores
            for label, feature, median_diff, pvalue in drug_features_scored:
                # Skip if the feature was already processed
            ...
            # Generate input for scoring all features
            ...
            features_scored = [
                feat for feat_list in features_scored for feat in feat_list
            ]

            # Add all features to feature scores
            ...
            # Sort features for each cluster by p-value
            for label in self.feature_scores_per_cluster:
                self.feature_scores_per_cluster[label].sort(key=lambda x: x[2])
    ...
    def write_feature_score_per_cluster(self):
        """
        Write feature scores per cluster to correlated and anticorrelated files,
        ensuring only features starting with "drug" are included in the output.
        """
        ...
        # Dictionary to track features
        feature_seen = {}

        # Only process features that start with "drug"
        for label in self.feature_scores_per_cluster:
            for feature, median_diff, pvalue in self.feature_scores_per_cluster[label]:
                if not feature.startswith("drug"):
                    continue  # Skip non-drug features

                # Include feature in output if not already processed
                if feature in feature_seen:
                    # Compare p-values for the feature
                    recorded_pvalue, recorded_median_diff, recorded_label, recorded_file = feature_seen[feature]
                    if pvalue < recorded_pvalue:
                        # Overwrite with smaller p-value
                        feature_seen[feature] = (pvalue, median_diff, label,
                                                 f_file if median_diff > 0 else f_anti_file)
                    elif pvalue == recorded_pvalue:
                        # Retain existing entry (first occurrence logic)
                        continue
                else:
                    # First occurrence of the feature
                    feature_seen[feature] = (pvalue, median_diff, label,
                                             f_file if median_diff > 0 else f_anti_file)

        # Write only drug features to the output files
        for feature, (pvalue, median_diff, label, file) in feature_seen.items():
        ...
        if self.survival_feature_scores_per_cluster:
            ...
            for label in self.survival_feature_scores_per_cluster:
                for feature, pvalue in self.survival_feature_scores_per_cluster[label]:
                    if not feature[0].startswith("drug"):
                        continue  # Skip non-drug features
            ...
            print("No survival features detected. File: {0} not written".format(f_file_name))
